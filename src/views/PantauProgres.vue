...
<template>
  <div class="space-y-8 pb-10 relative">
    ...
    <div class="transition-colors duration-300">
      ...
      <p class="text-slate-500 dark:text-slate-400 mt-2">Pantau level produktivitas dan daftar Self Reward-mu di sini.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-between hover:-translate-y-1 transition-all duration-300">
        ...
        <div class="w-12 h-12 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-500 dark:text-blue-400 flex items-center justify-center">
          <Brain class="w-6 h-6" />
        </div>
      </div>
      <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-between hover:-translate-y-1 transition-all duration-300">
      ...
      <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-between hover:-translate-y-1 transition-all duration-300">
        ...
        <div class="w-12 h-12 rounded-full bg-indigo-50 dark:bg-indigo-500/10 text-indigo-500 dark:text-indigo-400 flex items-center justify-center">
      ...
      <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-between hover:-translate-y-1 transition-all duration-300">
        ...
        <div class="w-12 h-12 rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-500 dark:text-emerald-400 flex items-center justify-center">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="bg-slate-900 dark:bg-slate-800 p-8 rounded-3xl shadow-sm text-white relative overflow-hidden transition-all duration-300 hover:shadow-lg group">
        <div class="absolute top-0 right-0 w-32 h-32 bg-emerald-500 rounded-full blur-[80px] opacity-40 group-hover:scale-125 transition-transform duration-700"></div>
        <div class="relative z-10 flex items-center gap-6 mb-8">
          <div class="w-20 h-20 rounded-full bg-slate-800 dark:bg-slate-700 border-4 border-emerald-400 flex items-center justify-center shadow-[0_0_20px_rgba(16,185,129,0.3)]">
            <span class="text-3xl font-black text-emerald-400">{{ levelSaatIni }}</span>
          </div>
          <div>
            <h2 class="text-2xl font-bold">Level Peneliti</h2>
            <p class="text-slate-400 text-sm mt-1">Total EXP: <span class="font-bold text-white">{{ totalEXP }}</span></p>
          </div>
        </div>
      ...
      <div class="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors duration-300">
        ...
        <div v-if="selfRewards.length === 0" class="text-center py-8 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
          <p class="text-slate-500 dark:text-slate-400 text-sm font-medium px-4">Belum ada Reward yang ditentukan.<br>Coba isi di form Kanban atau Revisi!</p>
        </div>

        <div v-else class="grid grid-cols-2 gap-4 max-h-64 overflow-y-auto pr-2">
          <div v-for="reward in selfRewards" :key="reward.id" class="p-4 rounded-2xl border transition-all flex flex-col items-center text-center group relative overflow-hidden h-full" :class="reward.terbuka ? 'border-emerald-200 dark:border-emerald-500/30 bg-emerald-50/50 dark:bg-emerald-500/5' : 'border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 grayscale opacity-60'">
            
            <div class="w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-transform duration-500" :class="[reward.warna, reward.terbuka ? 'group-hover:rotate-12 group-hover:scale-110' : '']">
              <Gift class="w-6 h-6" />
            </div>
    ...
    <div v-if="showPerayaan" class="fixed inset-0 z-[100] flex items-center justify-center overflow-hidden">
      ...
      <div class="modal-bounce bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center relative overflow-hidden border border-slate-200 dark:border-slate-700 mx-4 z-10">
        ...
        <div class="relative z-20 flex flex-col items-center">
          ...
          <p class="text-slate-500 dark:text-slate-400 font-medium mb-8 leading-relaxed">{{ dataPerayaan.deskripsi }}</p>
          
          <button @click="showPerayaan = false" class="w-full py-3 px-4 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl transition-all hover:scale-105 hover:-translate-y-1 shadow-[0_4px_14px_0_rgba(16,185,129,0.39)] flex justify-center items-center">
            Klaim Hadiah <PartyPopper class="w-5 h-5 ml-2" />
          </button>
        </div>
      </div>
    </div>
...
<style scoped>
.modal-bounce { animation: bounceIn 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }
@keyframes bounceIn {
...
.icon-pop { animation: popIcon 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275) 0.2s both; }
@keyframes popIcon {
...
.confetti-piece {
  position: absolute; top: -20px; width: 10px; height: 20px; opacity: 0; border-radius: 2px; animation: fallDown linear forwards;
}
@keyframes fallDown {
