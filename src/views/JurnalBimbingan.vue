...
const tambahKeGCal = (item) => {
	...
	const detail = encodeURIComponent(
		`Pengingat otomatis dari KawalSkripsi.\nTipe: ${item.tipe}`,
	);
...
const zonaWaktuLokal = computed(() => {
	...
	try {
		const waktuString = new Date().toLocaleTimeString("id-ID", {
			timeZoneName: "short",
		});
...
<template>
	<div class="space-y-12">
		<div
			class="border-b border-slate-200 dark:border-slate-700 pb-6 transition-colors duration-300">
			<h1
				class="text-3xl font-bold text-slate-800 dark:text-white flex items-center">
				<BookOpenCheck class="mr-3 text-emerald-500 w-8 h-8" />
				Jurnal Bimbingan
			</h1>
			<p class="text-slate-500 dark:text-slate-400 mt-2">
				Atur jadwal bimbingan dan catatan. Tentukan <b>Self Reward</b> untuk
				setiap revisi agar tetap semangat!
			</p>
		</div>
		...
		<section>
			<div class="flex items-center justify-between mb-6">
				<h2
					...
					<CalendarDays
						class="w-6 h-6 mr-2 text-slate-400 dark:text-slate-500" />
					Agenda Terdekat
				</h2>
				<button
					@click="isFormJadwalBuka = !isFormJadwalBuka"
			...
			<div
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<input
						v-model="formJadwal.topik"
						type="text"
						placeholder="Topik (misal: Cek Bab 4)"
					...
					<div class="relative group">
						...
						<input
							v-model="formJadwal.tanggal"
							type="date"
							@click="bukaPicker"
							@focus="bukaPicker"
							class="absolute inset-0 w-full h-full opacity-0 cursor-pointer dark:scheme-dark z-10" />
					</div>
					...
					<div class="relative group">
						<div
							...
							<span
								class="flex items-center text-sm font-medium"
								:class="
									formJadwal.waktu ? '' : 'text-slate-400 dark:text-slate-500'
								">
								<Clock class="w-4 h-4 mr-2 opacity-70" />
								{{
									formJadwal.waktu
										? formJadwal.waktu + " " + zonaWaktuLokal
										: "Pilih Waktu"
								}}
							</span>
						</div>
					...
					<div class="relative z-20">
						<div
							v-if="isDropdownTipeBuka"
							@click="isDropdownTipeBuka = false"
						...
						<div class="relative z-20 h-full">
							<button
								...
								<ChevronDown
									...
									:class="{ 'rotate-180': isDropdownTipeBuka }" />
							</button>

							<transition name="fade">
								<div
									v-if="isDropdownTipeBuka"
									class="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden py-1 z-30">
									<button
										type="button"
			...
			<div v-else class="flex overflow-x-auto pb-4 gap-4 snap-x">
				<div
					...
					:key="item.id"
					class="snap-start shrink-0 w-80 bg-white dark:bg-slate-800 rounded-2xl border shadow-sm flex flex-col overflow-hidden transition-colors duration-300"
					:class="
						(item.status || 'pending') === 'selesai'
							? 'border-emerald-200 dark:border-emerald-700/50 opacity-70'
					...
					<div class="p-5 flex-1">
						<div class="flex justify-between items-start mb-3">
							<span
								class="text-xs font-bold px-2 py-1 rounded bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors">
								{{ formatTanggal(item.tanggal) }}
							</span>
							<span
								v-if="(item.status || 'pending') === 'selesai'"
								class="text-xs font-bold px-2 py-1 rounded bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 transition-colors">
								Selesai
							</span>
							<span
								v-else
								class="text-xs font-bold px-2 py-1 rounded bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 transition-colors">
								Pending
							</span>
						</div>

						<h3
							class="font-bold text-slate-800 dark:text-white mb-2 truncate transition-colors"
							:class="
								(item.status || 'pending') === 'selesai'
						...
						<div
							class="text-xs text-slate-500 dark:text-slate-400 space-y-1.5 font-medium transition-colors">
							<div class="flex items-center">
								<Clock class="w-3.5 h-3.5 mr-1.5" />{{ item.waktu }}
								{{ zonaWaktuLokal }}
							</div>
							<div class="flex items-center">
								<component
									:is="item.tipe === 'Online' ? Video : MapPin"
						...
						<div
							class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700 flex justify-between gap-2 items-center transition-colors">
							<button
								@click="tambahKeGCal(item)"
								class="flex items-center text-xs font-medium text-blue-500 dark:text-blue-400 hover:text-blue-600 dark:hover:text-blue-300 bg-blue-50 dark:bg-blue-500/10 px-2 py-1.5 rounded transition-colors"
							...
							<div class="flex gap-1.5">
								<button
									v-if="(item.status || 'pending') !== 'selesai'"
									@click="ubahStatusJadwal(item.id, 'selesai')"
									class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-emerald-500 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded transition-colors"
									title="Tandai Selesai">
									<CheckCircle2 class="w-4 h-4" />
								</button>
								<button
									v-if="(item.status || 'pending') === 'selesai'"
									@click="ubahStatusJadwal(item.id, 'pending')"
									class="p-1.5 text-emerald-500 dark:text-emerald-400 hover:text-amber-500 dark:hover:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-500/10 rounded transition-colors"
									title="Batalkan / Tandai Pending">
									<CircleDashed class="w-4 h-4" />
								</button>
								<button
									type="button"
									@click="bukaModalHapusJadwal(item.id)"
									class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded transition-colors"
									title="Hapus">
		...
		<section>
			<div
				...
				<h2
					class="text-xl font-bold text-slate-800 dark:text-white flex items-center transition-colors">
					<PenLine class="w-6 h-6 mr-2 text-slate-400 dark:text-slate-500" />
					Buku Catatan
				</h2>

				<div
					class="flex items-center p-1 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700 w-fit transition-colors">
					<button
						@click="tabCatatan = 'revisi'"
			...
			<div
				class="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border mb-6 transition-colors duration-300"
				:class="
					tabCatatan === 'revisi'
						? 'border-slate-200 dark:border-slate-700'
				...
				<div class="space-y-3">
					<input
						v-model="judulCatatan"
						type="text"
						:placeholder="
							tabCatatan === 'revisi'
								? 'Topik Revisi (misal: Perbaikan Bab 3)'
								: 'Judul Catatan (misal: Ide Judul Skripsi)'
						"
					...
					<textarea
						...
						:placeholder="
							tabCatatan === 'revisi'
								? 'Detail masukan dari dosen...'
								: 'Tuliskan idemu di sini...'
						"
					...
					<div v-if="tabCatatan === 'revisi'" class="flex items-center gap-3">
						...
						<input
							v-model="rewardCatatan"
							type="text"
							placeholder="Self Reward jika tuntas (misal: Beli Kopi / Main Game 2 Jam)"
							class="w-full px-4 py-2 bg-rose-50/50 dark:bg-rose-900/10 border border-rose-200 dark:border-rose-800/50 text-slate-800 dark:text-white placeholder-rose-400/70 rounded-xl outline-none focus:ring-2 focus:ring-rose-400 transition-colors" />
					</div>

					<div class="flex justify-end pt-2">
						<button
							@click="tambahCatatan"
							:disabled="!judulCatatan || !isiCatatan"
			...
			<div
				...
				{{
					tabCatatan === "revisi"
						? "Skripsimu aman! Belum ada catatan revisi"
						: "Belum ada catatan"
				}}
			</div>
			...
			<TransitionGroup
				...
				<div
					...
					<div
						v-if="isCatatanSelesai(item)"
						class="absolute top-4 right-4 text-emerald-500 bg-emerald-100 dark:bg-emerald-500/20 px-2 py-1 rounded text-xs font-bold flex items-center">
						<CheckCircle2 class="w-3 h-3 mr-1" /> Tuntas
					</div>

					<h3
						class="font-bold text-lg text-slate-800 dark:text-white mb-2 transition-colors pr-16"
						:class="
							isCatatanSelesai(item) ? 'line-through decoration-slate-400' : ''
						">
						{{ item.judul }}
					</h3>
					<p
						class="text-slate-600 dark:text-slate-300 text-sm whitespace-pre-wrap flex-1 transition-colors"
						:class="item.reward ? 'mb-4' : 'mb-6'">
						{{ item.isi }}
					</p>

					<div
						v-if="item.reward"
						class="mb-5 mt-auto bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 px-3 py-2 rounded-lg text-xs font-medium flex items-center border border-rose-100 dark:border-rose-500/20">
						<Gift class="w-3.5 h-3.5 mr-2" />
						Reward: {{ item.reward }}
					</div>

					<div
						class="flex justify-between items-center pt-4 border-t border-slate-100 dark:border-slate-700 transition-colors"
						:class="!item.reward ? 'mt-auto' : ''">
						<span
							class="text-xs text-slate-400 dark:text-slate-500 flex items-center">
							<CalendarClock class="w-3 h-3 mr-1" />{{ item.tanggal }}
						</span>
						<button
							type="button"
							@click="bukaModalHapusCatatan(item.id)"
							class="text-slate-300 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 transition-colors p-1.5 rounded hover:bg-red-50 dark:hover:bg-red-500/10">
							<Trash2 class="w-4 h-4" />
						</button>
					</div>
				</div>
		...
		<transition name="fade">
			<div
				...
				<div
					...
					@click="batalHapus"></div>
				<div
					class="bg-white dark:bg-slate-800 w-full max-w-sm p-6 rounded-3xl shadow-2xl relative z-10 transition-colors duration-300 animate-in zoom-in-95 text-center border border-slate-200 dark:border-slate-700">
					<div
						class="w-16 h-16 rounded-full bg-red-50 dark:bg-red-500/10 flex items-center justify-center mx-auto mb-5 text-red-500 border border-red-100 dark:border-red-500/20 shadow-inner">
					...
					<p
						class="text-slate-500 dark:text-slate-400 mb-8 text-sm leading-relaxed">
						{{ deleteModal.desc }}
					</p>
					<div class="flex gap-3">
						<button
							@click="batalHapus"
							class="flex-1 py-3 px-4 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 font-bold rounded-xl transition-colors">
							Batal
						</button>
						<button
							@click="konfirmasiHapus"
							class="flex-1 py-3 px-4 bg-red-500 hover:bg-red-600 text-white font-bold rounded-xl transition-all shadow-[0_4px_14px_0_rgba(239,68,68,0.39)] hover:shadow-[0_6px_20px_rgba(239,68,68,0.23)] hover:-translate-y-0.5">
							Ya, Hapus
						</button>
					</div>
				</div>
