...
const MODES = {
  ...
  shortBreak: { nama: 'Rebahan Bentar', waktu: 0 * 60, ikon: Coffee, warna: 'bg-blue-500', teks: 'text-blue-500' },
...
// State Layar Penuh & Objek Audio
const isWaktuHabis = ref(false)
let audioAlarm = null // Menyimpan objek audio agar bisa dimatikan nanti
...
// Fungsi Mainkan Suara (Versi Annoying / Mengganggu)
const mainkanSuara = () => {
  // Menggunakan suara jam beker digital yang melengking
  audioAlarm = new Audio('/audio/alarm.mp3')

  // Set volume ke maksimal (1.0 = 100%)
  audioAlarm.volume = 1.0;
  
  // PENTING: Suara akan di-looping (terus berulang) tanpa henti!
...
const mulaiTimer = () => {
  ...
  interval = setInterval(() => {
    if (waktuTersisa.value > 0) {
      ...
      hentikanTimer()
      mainkanSuara() // Bunyikan alarm berisik
      isWaktuHabis.value = true // Munculkan layar
...
// Fungsi menutup layar selesai & Mematikan Suara
const tutupLayarSelesai = () => {
  isWaktuHabis.value = false
  resetTimer()
  
  // Matikan paksa alarm jika sedang berbunyi
  if (audioAlarm) {
    audioAlarm.pause()
    audioAlarm.currentTime = 0 // Kembalikan ke detik ke-0
  }
}

onUnmounted(() => {
  hentikanTimer()
  // Keamanan tambahan: matikan suara jika user malah pindah halaman saat alarm bunyi
  if (audioAlarm) {
    audioAlarm.pause()
  }
})
  ...
  <div class="space-y-8 flex flex-col items-center justify-center min-h-[80vh] relative">
    ...
    <div class="text-center">
      <h1 class="text-3xl font-bold text-slate-800 dark:text-white flex items-center justify-center transition-colors">
        <Timer class="mr-3 w-8 h-8" :class="MODES[modeAktif]?.teks" />
        Waktu Fokus
      </h1>
      <p class="text-slate-500 dark:text-slate-400 mt-2 max-w-md mx-auto transition-colors">
        Gunakan teknik Pomodoro untuk menjaga konsentrasi saat menyusun skripsi tanpa merasa kelelahan.
      </p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 p-6 sm:p-8 md:p-12 rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-slate-700 w-full max-w-lg text-center transition-all duration-500 relative overflow-hidden" :class="isBerjalan ? 'shadow-lg scale-[1.02]' : ''">
      ...
      <div class="flex flex-wrap justify-center gap-2 mb-8 md:mb-10 relative z-10">
        <button 
          v-for="(config, kunci) in MODES" 
          :key="kunci"
          @click="ubahMode(kunci)"
          class="px-4 py-2 rounded-full text-sm font-medium transition-all flex items-center"
          :class="modeAktif === kunci ? `${config.warna} text-white shadow-md` : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'"
        >
      ...
      <div class="relative z-10 font-mono text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-bold tracking-tighter mb-8 md:mb-10 transition-colors duration-500" :class="MODES[modeAktif]?.teks">
        {{ formatWaktu }}
      </div>
      ...
      <div class="flex items-center justify-center gap-4 relative z-10">
        <button 
          v-if="!isBerjalan" 
          @click="mulaiTimer"
          class="flex items-center justify-center w-16 h-16 md:w-20 md:h-20 rounded-full bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-white hover:scale-105 transition-all shadow-md"
        >
        ...
        <button 
          v-else 
          @click="hentikanTimer"
          class="flex items-center justify-center w-16 h-16 md:w-20 md:h-20 rounded-full bg-red-500 text-white hover:bg-red-600 hover:scale-105 transition-all shadow-md animate-pulse"
        >
        ...
        <button 
          @click="resetTimer"
          class="flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 hover:rotate-180 transition-all duration-300"
          title="Reset Waktu"
        >
    ...
    <transition name="layar-selesai">
      <div 
        v-if="isWaktuHabis" 
        class="fixed inset-0 z-100 flex flex-col items-center justify-center text-white p-6 text-center transition-colors duration-500"
        :class="MODES[modeAktif]?.warna"
      >
        <div class="animate-in zoom-in slide-in-from-bottom-8 duration-700 flex flex-col items-center">
          ...
          <h1 class="text-5xl md:text-7xl font-black mb-4 tracking-tight drop-shadow-lg">
            Waktu Selesai!
          </h1>
          <p class="text-lg md:text-2xl font-medium opacity-90 mb-10 max-w-md drop-shadow-md">
            {{ modeAktif === 'pomodoro' ? 'Kerja bagus! Sesi fokusmu telah selesai. Jangan lupa peregangan ya!' : 'Waktu istirahat habis. Ayo kembali fokus mengerjakan skripsi!' }}
          </p>
          
          <button 
            @click="tutupLayarSelesai"
            class="px-8 py-4 bg-white text-slate-900 font-bold text-lg rounded-full shadow-xl hover:scale-105 active:scale-95 transition-all"
          >
            Tutup & Lanjut
          </button>
        </div>
...
<style scoped>
.layar-selesai-enter-active,
.layar-selesai-leave-active {
  transition: all 0.5s ease;
}
.layar-selesai-enter-from,
.layar-selesai-leave-to {
  opacity: 0;
